...
class RNNCell(object):
    ...
    def backward(self, delta, h, h_prev_l, h_prev_t):
        ...
        self.dW_ih += np.dot(dz.T, h_prev_l)/batch_size #  (hidden_size, batch_size)*(batch_size, input_size)  = (hidden_size, input_size)
        self.dW_hh += np.dot(dz.T, h_prev_t)/batch_size #  (hidden_size, batch_size)*(batch_size, hidden_size)  = (hidden_size, hidden_size)
        self.db_ih += np.sum(dz,axis=0)/batch_size
        self.db_hh += np.sum(dz,axis=0)/batch_size

        # 2) Compute dx, dh
        dx = np.dot(dz, self.W_ih) # (batch_size, hidden_size) * (hidden_size, input_size) = (batch_size, input_size)
